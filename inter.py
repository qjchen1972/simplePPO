...
class Interactive(abc.ABC):
    ...
    def _update(self, dt, path):
        ...
        while self._sim_time < self._current_time:
            ...
            #键B保存state
            if 'B' in keys:
                print(f"try to save state:{path}")
                self.save_state_to_file(path) #"Champion.Level12.RyuVsBison.2Player.state")
    ...
    def run(self, path):
        ...
        while True:
            ...
            now = time.time()
            self._update(now - prev_frame_time, path)
            prev_frame_time = now
            self._draw()
            self._win.flip()
...
def savestate(path="data/1player.state"):
    '''
    ...
    #ia = RetroInteractive(game=game, state=args.state, scenario=args.scenario)
    ia = RetroInteractive(game=game, state=state, scenario='scenario')    
    ia.run(path)
